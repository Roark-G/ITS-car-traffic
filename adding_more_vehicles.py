...
# this idea came from a stackoverflow post
...
class Simulation:
    ...
    # chatgpt'ed code saving/ loading
...
class Vehicle():
    ...
    def get_next_node_dist(self):
        ...
        return np.abs(np.hypot(target_node.x - self.x, target_node.y - self.y))
    
    def get_true_destination_dist(self):
        return np.abs(np.hypot(self.true_destination[0] - self.x, self.true_destination[1] - self.y))
    ...
    def a_star_stack(self, destination: RoadNode):
        ...
        start_node = min(
            ...
            key=lambda n: np.abs(np.hypot(n.x - self.x, n.y - self.y))
        )
        
        g_scores[start_node] = 0
        heapq.heappush(open_set, (0 + heuristic(start_node, destination), next(unique_id), start_node))
        ...
        while open_set:
            ...
            for neighbor in current_node.connections:
                tentative_g_score = g_scores[current_node] + np.abs(np.hypot(
                ...
                if tentative_g_score < g_scores[neighbor]:
                    ...
                    heapq.heappush(open_set, (
                        tentative_g_score + heuristic(neighbor, destination),
                        next(unique_id),
                        neighbor
                    ))    
    ...
    def update_true_destination(self):
        ...
        def is_car_en_route():
            ...
            for other in self.simulation.vehicles:
                if other == self or not other.node_stack:
                    continue

                if other.node_stack[0] != self.node_stack[0]:
                    continue

                other_distances.append(np.abs(np.hypot(other.x - self.x, other.y - self.y)))
...
def create_simple(save=False):
    sim = Simulation()

    A = RoadNode(10, SIZE_Y//2 - 10, label='A')
    B = RoadNode(10, SIZE_Y//2 + 10, label='B')
    C = RoadNode(SIZE_X - 10, SIZE_Y//2 - 10, label='C')
    D = RoadNode(SIZE_X - 10, SIZE_Y//2 + 10, label='D')
...
# I created this in a different project
...
if __name__ == "__main__":
    mysim = create_3_way(save=False)

    mysim.add_vehicle(Vehicle(mysim, SIZE_X//2 + 10, SIZE_Y//2 - 20))
    mysim.add_vehicle(Vehicle(mysim, 10, SIZE_Y//2 + 10))
    mysim.add_vehicle(Vehicle(mysim, SIZE_X//2 + 10, 10))

    create_video(mysim, r"videos\\3way_3cars", rate=1)